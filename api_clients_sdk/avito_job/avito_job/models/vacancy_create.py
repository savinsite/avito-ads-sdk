...
class VacancyCreate(BaseModel):
    ...
    VacancyCreate
    """ # noqa: E501
    address: Optional[StrictStr] = Field(default=None, description="Полный адрес объекта (строка длиной от 1 до 256 символов). Обязательное, если не указаны координаты.")
    ...
    description: StrictStr = Field(description="Описание вакансии (строка длиной от 1 до 5000 символов) Поддерживает html-тэги `p`, `ul`, `ol`, `li`, `br`, `strong`, `em`")
    ...
    employee_id: Optional[StrictInt] = Field(default=None, description="employee_id - Идентификатор сотрудника на Авито. Если этот параметр указан, то вакансия будет закреплена за сотрудником и с его баланса в Avito Pro будет списано размещение. Использовать параметр можно только с billing_type равным package. Сотрудник должен быть активен. ")
    employment: StrictStr = Field(description="Занятость <br> Возможные значения:   - temporary - Временная   - full - Полная   - internship - Стажировка   - partial - Частичная  Если ничего не выбрать то будет автоматически проставляться в зависимости от режима работы: При flexible и partTime, тип занятости - partial. Для всех остальных full. ")
    ...
    name: StrictStr = Field(description="Название вакансии (строка длиной от 1 до 50 символов)")
    payout_frequency: Optional[VacancyCreatePayoutFrequency] = None
    ...
    salary_range: Optional[VacancyCreateSalaryRange] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "name": obj.get("name"),
            "payout_frequency": VacancyCreatePayoutFrequency.from_dict(obj["payout_frequency"]) if obj.get("payout_frequency") is not None else None,
            ...
            "salary_range": VacancyCreateSalaryRange.from_dict(obj["salary_range"]) if obj.get("salary_range") is not None else None,
            "schedule": VacancyCreateSchedule.from_dict(obj["schedule"]) if obj.get("schedule") is not None else None,
