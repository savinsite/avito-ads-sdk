...
class VasIdRequestBody(BaseModel):
    """
    VasIdRequestBody
    """ # noqa: E501
    vas_id: StrictStr = Field(description="Идентификатор услуги, возможные его варианты значения: - `highlight` — [выделение объявления](https://support.avito.ru/articles/200026858) - `xl` – [XL-объявление](https://support.avito.ru/articles/685) ")
    __properties: ClassVar[List[str]] = ["vas_id"]

    @field_validator('vas_id')
    def vas_id_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['highlight', 'xl']):
            raise ValueError("must be one of enum values ('highlight', 'xl')")
        return value
