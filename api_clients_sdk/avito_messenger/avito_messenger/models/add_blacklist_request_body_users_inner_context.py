...
class AddBlacklistRequestBodyUsersInnerContext(BaseModel):
    """
    AddBlacklistRequestBodyUsersInnerContext
    """ # noqa: E501
    item_id: Optional[StrictInt] = None
    reason_id: Optional[StrictInt] = Field(default=None, description="причина, по которой блокируем пользователя, 1 - спам, 2 - мошенничество, 3 - оскорбления и хамство, 4 - другая причина")
    __properties: ClassVar[List[str]] = ["item_id", "reason_id"]

    @field_validator('reason_id')
    def reason_id_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
